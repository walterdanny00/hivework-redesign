...
## Section 55 — Session 44 (2026-09-04): Tokenization pass — status-pill/pill-track/tile-row colors, 11 files

Picked up the tokenization item carried from Sections 52-53. Started
scoped to the 4 files already flagged (`HistoryWork.tsx`,
`HistoryJobs.tsx`, `RangeFilter.tsx`, `Jobs.tsx`), but investigation blew
the scope open before any patch was written.

None of the 4 files declare their own `:root` block — raw hex throughout.
`HistoryWithdrawals.tsx`'s existing local `:root` redeclaration led to the
real global source: `~/Piwork/frontend/src/index.css`. `--pi-gold`
(`#B8860B`) already existed there but was never referenced via `var()`.
The real finding: the status-pill/pill-track trio —
`#FFF3DC`/`#B8860B` (pending), `#E4F8F6`/`#1A9E92` (open/verified),
`#F1EFEA`/`#6B6874` (closed) — plus `#EFECE5` (pill-track bg) repeats
untokenized across **11 files**, not 4: `Layout.tsx`, `Home.tsx`,
`Jobs.tsx`, `JobDetail.tsx` (worker + owner), `Dashboard.tsx`,
`Profile.tsx`, `HistoryJobs.tsx`, `HistoryWithdrawals.tsx` (hardcoded even
inside its own `:root`-bearing file), `HistoryWork.tsx`, `RangeFilter.tsx`.
No existing token matched any of these (`--mint` `#2EC4B6` is close to but
not `#1A9E92`). **User decision: expand to all 11 files, name new
tokens.**

8 new tokens added to `index.css`'s `:root`: `--pi-gold-tint` (`#FFF3DC`),
`--teal` (`#1A9E92`), `--teal-tint` (`#E4F8F6`), `--mist` (`#F1EFEA`),
`--sand` (`#EFECE5`), `--coral-tint` (`#FFE8E5`), `--violet-tint`
(`#F3E8FF`), `--sky-tint` (`#E8F0FF`). Flagged, not resolved: `--cream`
(`#F7F5F1`), `--line` (`#E7E3DA`), `--mist`, and `--sand` are four very
close near-white warm-grays — could be intentional layering or drift; not
merged, just named.

Patch applied via the standing backup + unique-anchor-check Python script
convention. **Bug hit and fixed:** `index.css`'s anchor line
(`--pi-gold: #B8860B;`) stayed a substring of its own replacement, so a
second script run still passed the "matches exactly once" check and
duplicated the 8 new token lines (16 insertions instead of 8). Caught via
`git diff`, fixed with `sed -i '35,42d' index.css`, re-checked clean.
Terminal-scrollback truncation also bit the first diff-review attempt
(hunks missing/duplicated in the paste) — worked around by writing
patch/diff output straight to files instead of relying on live scrollback.

Full 11-file diff reviewed hunk-by-hunk: every change a clean hex →
`var(--token)` swap, nothing else touched — `Home.tsx`'s unrelated
`.hw-status-pill.paid{#EFEAFB}` correctly left alone (out of scope);
`RangeFilter.tsx`'s swap correctly kept quoted JS string syntax
(`'var(--sand)'`) for its inline `style={{}}` usage. `npx tsc` clean.
`npm run build` clean: `325.40 kB` JS / `2.38 kB` CSS (up from session
41's `324.97 kB`/`2.23 kB` — expected, longer `var()` strings vs. short
hex, not a red flag).

Pushed to the Piwork repo (real code, `frontend/src/` only):
`refactor: tokenize status-pill/pill-track/tile-row colors to CSS custom
properties across 11 files`. **Push confirmed clean by the user.
Live spot-check (Dashboard status pills, Browse tile colors, Job Detail
owner status chips) confirmed good.**

Full detail: see session-44.md.

## Section 56 — Session 45 (2026-09-04): `.pj-combo input` background resolved

Picked up the open item carried from Section 53/55: no canonical answer
existed for whether `.pj-combo input` should have a background, and if so
`--cream` (`Profile.tsx`) or `--card` (`Onboarding.tsx`).

Checked the shell's analogous `.hwpc-combobox` canonical pattern first
(container gets `var(--card)`, inner input transparent) — promising, but
`Combobox.tsx` turned out not to follow that shape. Pulled `Combobox.tsx`
directly (shared component, extracted from `PostJob.tsx`'s original
`PJCombobox`) plus a `chip-row` grep across all three host files:
`.pj-combo` is a bare `position:relative` wrapper in all 3, the `<input>`
sits as a direct sibling (not nested in `chip-row`), and `chip-row` itself
is identically unstyled (layout-only) everywhere.

**The real finding:** `PostJob.tsx` — the origin file — never styled
`.pj-combo input` at all. No rule existed there. The live Post Job combo
input has always rendered as a bare, unstyled native input with no border,
padding, or background. Only `Profile.tsx` and `Onboarding.tsx` had each
independently patched in a background, in opposite directions, with no
origin file to arbitrate — why Section 53 couldn't resolve this the way it
resolved the other 3 `ProfileForm` token swaps.

**Decision: `--card`**, matching `Onboarding.tsx` and the broader
field-surface convention (`.field input`, and the `textarea`/`skills-box`
convention fixed to `--card` in Section 53). Added the missing
`.pj-combo input` rule + `:focus` state to `PostJob.tsx` (copied from
`Onboarding.tsx`), and swapped `Profile.tsx`'s `--cream` to `--card`.

Patch script, backup + unique-anchor-check convention. Diff clean — exactly
the two intended changes. `npx tsc` clean; `npm run build` clean:
`325.70 kB` JS (up slightly from session 44's `325.40 kB`, expected — new
CSS text) / `2.38 kB` CSS unchanged (inline JS-embedded styles, not
`index.css`).

Pushed to the Piwork repo: `fix: style .pj-combo input in PostJob.tsx (was
unstyled) and align Profile.tsx to --card, matching Onboarding/canonical
field-surface convention`. **Push confirmed clean by the user.**
...
## Section 88 (session 76) — Dashboard budget-tracker demo data reconciled with Section 43 (2026-09-10)

Picked up open item 1. Swept `hivework-app-v4-3.html`'s Dashboard demo
data against what Section 43 actually shipped to real `Dashboard.tsx`
and found two points of drift:

1. **Missing third stat pill.** Real code added a worker-tab-only
   "Pending" stat pill in Section 43, sourced from the real
   `earnings_pending` backend field (sum of approved-status
   application budgets). The shell's `.dash-stat-row` only ever had
   two pills ("Jobs done"/"Jobs posted" swap + static "Rating") and
   never picked this up. **Fixed:** added a third `.stat-pill`
   (`#dash-stat3`), gated the same way as `HW_DASH_CLOSED_JOB` —
   visible on the My Work (worker) tab, hidden entirely on My Jobs
   (client) via `toggleWork()`. New demo constant
   `HW_DASH_EARNINGS_PENDING = 3` (a plain π figure, not derived from
   other demo state — there's no existing pending-application demo
   data to derive it from).
2. **Stale comment, not a functional bug.** `HW_DASH_JOBS_POSTED_COUNT`'s
   comment claimed real code has no jobs-posted-count field anywhere —
   true when Section 34 first wrote it, but Section 43 closed that gap
   for real (`jobs_posted_count`, a Supabase
   `.select('id', {count:'exact', head:true}).eq('client_id', user_id)`
   query). The shell's locally-derived count (`2 +
   (HW_DASH_CLOSED_JOB.refunded ? 1 : 0)`) still renders the same
   correct number — nothing to fix behaviorally — but the comment was
   corrected so it no longer describes this as "a genuine addition, not
   a ported fact" when it now demoes a real field.

Budget tracker card itself (Posted/Refunded/"Net committed") was
checked and found accurate to real code — no drift there, nothing
touched.

**Verification:** diffed edited file against original — 3 isolated
changes only (markup pill, two data-constant/comment edits, one
`toggleWork()` addition), nothing else touched. `lxml` parse clean;
div open/close balance unchanged from the file's pre-existing baseline
(the file has always been off-by-one due to `'</div>'` string literals
inside JS template strings, confirmed present in the original
unedited file too — not a real tag mismatch).
