...
/**
 * Hivework — Landing Page
 * Marketing/logged-out page. Drop into your router at "/".
 * All entry CTAs (nav "Get started", hero "Find work"/"Post a job") route
 * into /onboarding — hero buttons pass ?intent=find or ?intent=post so
 * onboarding can route into Browse vs the Post wizard afterward; nav's
 * CTA carries no intent and falls back to Home.
 *
 * Requires hivework-tokens.css to be imported somewhere in the app
 * (imported here directly, but importing it once at the app root also works
 * and avoids any duplicate-import warnings from your bundler).
 */
export default function HiveworkLanding() {
  ...
  return (
    <>
      <style>{`
        ...
        /* ---------- ESCROW TICKER (signature element) ---------- */
        ...
        /* ---------- FLOW (real sequence, numbered on purpose) ---------- */
        ...
        <div className="hero">
          <div>
            ...
            <div className="hero-ctas">
              <button className="btn btn-primary" onClick={() => window.location.href = "/onboarding?intent=find"}>Find work →</button>
              <button className="btn btn-ghost" onClick={() => window.location.href = "/onboarding?intent=post"}>Post a job</button>
            </div>
